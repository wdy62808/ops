...
net_devs=`ls -d /etc/sysconfig/network-scripts/ifcfg-*|grep -v 'ifcfg-lo'|grep -Eo 'ifcfg-(.+)'|cut -d '-' -f2`

for dev in `echo $net_devs` 
do
    ifconfig |grep $dev >>/dev/null
    if [ $? -eq 0 ];then #只针启用的网卡设置多队列属性
         echo "设置Dev:$dev 的多队列数值"
